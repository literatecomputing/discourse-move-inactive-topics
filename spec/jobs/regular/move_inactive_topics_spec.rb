...
RSpec.describe Jobs::MoveInactiveTopics do
  fab!(:user)
  fab!(:category) { Fabricate(:category_with_definition, user: user, description: "this is a great category") }
  fab!(:exempt_category) { Fabricate(:category_with_definition, user: user, description: "this is an exempt category") }
  ...
  it "should not move category descriptions" do
    category.update_column(:topic_id, description_topic.id)
    expect (category.topic_id.to_i) > 0
